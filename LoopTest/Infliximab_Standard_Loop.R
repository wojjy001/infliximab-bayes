...
# Source the other R scripts and execute
	work.dir <- "/Volumes/Prosecutor/PhD/InfliximabBayes/infliximab-bayes/LoopTest/"	# Mac directory
...
# ------------------------------------------------------------------------------
	filename <- paste0(work.dir,"covariate_changes.png")
	png(filename,width = 800,height = 800,pointsize = 10)
	...
	# Plot weight over time
		...
		plotobj1 <- plotobj1 + geom_line(aes(x = time,y = WTCOV),colour = "red")
		plotobj1 <- plotobj1 + scale_x_continuous("\nTime (days)")
		plotobj1 <- plotobj1 + scale_y_continuous("Total body weight (kg)\n")
		print(plotobj1,vp = vplayout(1:2,1:2))

	# Plot albumin over time
		plotobj2 <- NULL
		plotobj2 <- ggplot(optimise.bayes.data1[optimise.bayes.data1$time <= last.time,])
		plotobj2 <- plotobj2 + ggtitle("(b)\n")
		plotobj2 <- plotobj2 + geom_line(aes(x = time,y = ALBCOV),colour = "red")
		plotobj2 <- plotobj2 + scale_x_continuous("\nTime (days)")
		plotobj2 <- plotobj2 + scale_y_continuous("Albumin (U/L)\n")
		print(plotobj2,vp = vplayout(1:2,3:4))

	# Plot ADA status over time
		plotobj3 <- NULL
		plotobj3 <- ggplot(optimise.bayes.data1[optimise.bayes.data1$time <= last.time,])
		plotobj3 <- plotobj3 + ggtitle("(c)\n")
		plotobj3 <- plotobj3 + geom_step(aes(x = time,y = ADA),colour = "red")
		plotobj3 <- plotobj3 + scale_x_continuous("\nTime (days)")
		plotobj3 <- plotobj3 + scale_y_continuous("Presence of anti-drug antibodies\n",breaks = c(0,1),labels = c(0,1))
		print(plotobj3,vp = vplayout(3:4,1:2))

	# Plot concentrations over time
		plotobj4 <- NULL
		plotobj4 <- ggplot(optimise.bayes.data1[optimise.bayes.data1$time <= last.time,])
		plotobj4 <- plotobj4 + ggtitle("(d)\n")
		plotobj4 <- plotobj4 + geom_line(aes(x = time,y = IPRE),colour = "red")
