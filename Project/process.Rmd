...
# Set working directory
  ...
  nsim <- 100 # Number of simulations of seed individuals
...
```{r Bayes Scenario NTimeWeightAllCov Plot Output,echo = FALSE,warning = FALSE}
...
```{r Bayes Scenario NTimeWeightAllCov AUT Output,echo = FALSE,warning = FALSE}
# Calculate the time spent under the target trough at the end of each sampling interval
  ...
  print(bayes.AUT.summary1[bayes.AUT.summary1$time == 546,])
```

### Peck 1.005 Time-Weighting, All Covariate Information is available

All observations are time-weighted using the Peck method with Q = 1.005 when maximising the posterior likelihood.  Covariate information at the time of sampling as available.

```{r Bayes Scenario Peck1.005AllCov Plot Output,echo = FALSE,warning = FALSE}
# Source the results of bayes_simulation
  method2 <- "Peck1.005"
  covariate2 <- "AllCov"
...
# Plot the median and 95% prediction intervals for individual predicted concentrations (IPRE) over time
  plotobj4 <- NULL
  plotobj4 <- ggplot()
  plotobj4 <- plotobj4 + stat_summary(aes(x = time,y = IPRE),data = bayes.data2,geom = "line",fun.y = median,colour = "darkgreen")
  plotobj4 <- plotobj4 + stat_summary(aes(x = time,y = IPRE),data = bayes.data2,geom = "ribbon",fun.ymin = "CI95lo",fun.ymax = "CI95hi",fill = "darkgreen",alpha = 0.3)
...
```{r Bayes Scenario Peck1.005AllCov AUT Output,echo = FALSE,warning = FALSE}
# Calculate the time spent under the target trough at the end of each sampling interval
  bayes.data.AUT2 <- bayes.data2[bayes.data2$time %in% sample.times,]
  bayes.AUT.summary2 <- ddply(bayes.data.AUT2, .(time), function(bayes.data.AUT2) summary.function(bayes.data.AUT2$AUT))
  print(bayes.AUT.summary2[bayes.AUT.summary2$time == 546,])
```

### Peck 1.01 Time-Weighting, All Covariate Information is available

All observations are time-weighted using the Peck method with Q = 1.01 when maximising the posterior likelihood.  Covariate information at the time of sampling as available.

```{r Bayes Scenario Peck1.01AllCov Plot Output,echo = FALSE,warning = FALSE}
# Source the results of bayes_simulation
  method3 <- "Peck1.01"
  covariate3 <- "AllCov"
...
# Plot the median and 95% prediction intervals for individual predicted concentrations (IPRE) over time
  plotobj5 <- NULL
  plotobj5 <- ggplot()
  plotobj5 <- plotobj5 + stat_summary(aes(x = time,y = IPRE),data = bayes.data3,geom = "line",fun.y = median,colour = "darkgreen")
  plotobj5 <- plotobj5 + stat_summary(aes(x = time,y = IPRE),data = bayes.data3,geom = "ribbon",fun.ymin = "CI95lo",fun.ymax = "CI95hi",fill = "darkgreen",alpha = 0.3)
...
```{r Bayes Scenario Peck1.01AllCov AUT Output,echo = FALSE,warning = FALSE}
# Calculate the time spent under the target trough at the end of each sampling interval
  bayes.data.AUT3 <- bayes.data3[bayes.data3$time %in% sample.times,]
  bayes.AUT.summary3 <- ddply(bayes.data.AUT3, .(time), function(bayes.data.AUT3) summary.function(bayes.data.AUT3$AUT))
  print(bayes.AUT.summary3[bayes.AUT.summary3$time == 546,])
```
...
```{r,echo = FALSE,warning = FALSE}
  label.data.AUT$SCEN <- 1
  clinical.data.AUT$SCEN <- 2
  bayes.data.AUT1$SCEN <- 3
  bayes.data.AUT2$SCEN <- 4
  bayes.data.AUT3$SCEN <- 5
  ...
  # plotobj6 <- plotobj6 + scale_y_log10("Time under target trough (days)\n",labels = scale.log10.labels,breaks = scale.log10.labels)
...
# Proportion of individuals below target trough concentration
...
```{r,echo = FALSE,warning = TRUE}
  ...
  below.target.546.label.data <- below.target.546(label.data)
  below.target.546.label.data
  below.target.546.clinical.data <- below.target.546(clinical.data)
  below.target.546.bayes.data1 <- below.target.546(bayes.data1)  
  below.target.546.bayes.data2 <- below.target.546(bayes.data2)  
  below.target.546.bayes.data3 <- below.target.546(bayes.data3)  
```
